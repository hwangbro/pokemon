...
public partial class Tcg {
    ...
    public int ClearText() {
        ...
        while(true) {
            ...
            if(ret == SYM["WaitForButtonAorB"] || ret == SYM["WaitForWideTextBoxInput.wait_A_or_B_loop"]) {
                ...
                AdvanceFrame();
            } else if(ret == SYM["DisplayCardList.wait_button"] && CpuRead("wWhoseTurn") != 0) {
                Press(Joypad.B);
            } else {
                return ret;
            }
    ...
    public bool DoTurn() {
        ...
        if(MyDeck.Hand.Contains(TrainerCards["Bill"]) && (active.Substatus3 & 0x2) != 2) {
            UseHandCard(MyDeck.Hand.IndexOf(TrainerCards["Bill"]));
            ClearText();
        }
        if(MyDeck.Hand.Contains(TrainerCards["Potion"]) && CpuRead("wPlayerArenaCardHP") != active.CurHP && (active.Substatus3 & 0x2) != 2) {
        ...
        */
        ...
        if(active.Status != TcgDuelStatus.None && MyDeck.Hand.Contains(TrainerCards["Full Heal"]) && (active.Substatus3 & 0x2) != 2) {
        ...
        if(active.Status != TcgDuelStatus.None && MyDeck.Hand.Contains(TrainerCards["Switch"]) && retreatIdx != -1 && (active.Substatus3 & 0x2) != 2) {
        ...
        // pluspower
        if(MyDeck.Hand.Contains(TrainerCards["PlusPower"]) && (active.Substatus3 & 0x2) != 2) {
            bool usePP = false;
            for(byte i = 0; i < 2; i++) {
        ...
        if(MyDeck.Hand.Contains(TrainerCards["Professor Oak"]) && (active.Substatus3 & 0x2) != 2) {
